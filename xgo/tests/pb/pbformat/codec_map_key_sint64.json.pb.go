...
func (x *CodecMapKeySInt64) UnmarshalJSON(b []byte) error {
	if x == nil {
		return errors.New("json: Unmarshal: xgo/tests/pb/pbformat.(*CodecMapKeySInt64) is nil")
	}
	var (
		err    error
		isNULL bool
...
LOOP_SCAN:
	for { // Loop to read the JSON objects
		...
		case "f_enum_key_numeric_val_enum_string":
			if x.FEnumKeyNumericValEnumString, err = jsondecoder.ReadMapI64EnumStr(dec, x.FEnumKeyNumericValEnumString, false, pbexternal.EnumNum1_value); err != nil {
				return err
			}
		...
		case "f_enum_key_string_val_enum_string":
			if x.FEnumKeyStringValEnumString, err = jsondecoder.ReadMapI64EnumStr(dec, x.FEnumKeyStringValEnumString, true, pbexternal.EnumNum1_value); err != nil {
				return err
			}
