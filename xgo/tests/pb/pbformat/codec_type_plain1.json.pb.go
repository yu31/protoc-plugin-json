...
func (x *CodecTypePlain1) UnmarshalJSON(b []byte) error {
	if x == nil {
		return errors.New("json: Unmarshal: xgo/tests/pb/pbformat.(*CodecTypePlain1) is nil")
	}
	var (
		err    error
		isNULL bool
...
LOOP_SCAN:
	for { // Loop to read the JSON objects
		...
		case "f_enum_string":
			if x.FEnumString, err = jsondecoder.ReadValEnumStr(dec, x.FEnumString, pbexternal.EnumNum1_value); err != nil {
				return err
			}
		case "f_any_native":
