...
func (x *MessageLevel05) UnmarshalJSON(b []byte) error {
	if x == nil {
		return jsondecoder.ErrStructIsNIL("xgo/tests/pb/pbinline", "MessageLevel05")
	}
	var (
		err    error
		isNULL bool
...
LOOP_SCAN:
	for { // Loop to read the JSON objects
		...
		case "level05_c_p_string1":
			if x.Level05PString1, err = jsondecoder.ReadPtrStr(dec, x.Level05PString1); err != nil {
				return err
			}
		case "level05_p_string2":
			if x.Level05PString2, err = jsondecoder.ReadPtrStr(dec, x.Level05PString2); err != nil {
				return err
			}
