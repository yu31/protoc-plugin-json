...
func (x *MessageLevel34) UnmarshalJSON(b []byte) error {
	if x == nil {
		return jsondecoder.ErrStructIsNIL("xgo/tests/pb/pbinline", "MessageLevel34")
	}
	var (
		err    error
		isNULL bool
...
LOOP_SCAN:
	for { // Loop to read the JSON objects
		...
		case "level34_c_p_string1":
			if x.Level34PString1, err = jsondecoder.ReadPtrStr(dec, x.Level34PString1); err != nil {
				return err
			}
		case "level34_p_string2":
			if x.Level34PString2, err = jsondecoder.ReadPtrStr(dec, x.Level34PString2); err != nil {
				return err
			}
